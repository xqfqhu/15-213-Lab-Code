    ...
    for (int i = list_offset; i < SEG_NUM; i++){
        ...
        while (rover != NULL){
            ...
            if (get_blockSize(rover->header) >= block_byte_aligned) {
                ...
                PER_PRINTF("%lu bytes allocated after %d blocks visited\n", size, block_visited);
                return rover->payload;
            }
            rover = rover->succ;
        }
    }
    PER_PRINTF("no block found for %lu bytes after %d blocks visited\n", size, block_visited);
    block_t * new_block;
...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    (size < copySize)? (copySize = size) : (copySize = copySize);

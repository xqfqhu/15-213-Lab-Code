...
#ifdef ANALYSIS 
#define ANALYSIS_PRINTF(...) fprintf(stdout, __VA_ARGS__)
#else
#define ANALYSIS_PRINTF(...)
#endif
...
static trace_t *read_trace(char *tracedir, char *filename)
{
	...
	sprintf(msg, "Could not open file in read_trace");
	unix_error(msg);
    }    
    ...
    fclose(tracefile);
    assert(((u_int64_t) max_index) == ((u_int64_t)(trace->num_ids - 1)));
    assert(((u_int64_t) (trace->num_ops)) == ((u_int64_t) op_index));
...
static int eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges) 
{
	...
	#ifdef ANALYSIS
    int total_size = 0;
	int heapsize_init = mem_heapsize();
	u_int64_t util = 0;
	#endif
	...
	size = trace->ops[i].size;
		...
		#ifdef ANALYSIS
	    total_size += size;
		util = 100 * total_size / (mem_heapsize() - heapsize_init);
		#endif
		ANALYSIS_PRINTF("%d byte allocated. memory util = %lu%%. \n", size, util);
	    /* 
	    ...
	    for (j = 0; j < oldsize; j++) {
	      if (newp[j] != (char) (index & 0xFF)) {
			  fprintf(stdout, "orginal data: %d, cur data: %d", (char) (index & 0xFF), newp[j]);
		...
		#ifdef ANALYSIS
		total_size = total_size - oldsize + size;
		util = 100 * total_size/(mem_heapsize() - heapsize_init);
		#endif
		ANALYSIS_PRINTF("%d byte reallocated. memory util = %lu%%\n", size, util);
	    /* Remember region */
        ...
        case FREE: /* mm_free */
		#ifdef ANALYSIS
		total_size = total_size - trace->block_sizes[index];
		util = 100 * total_size/(mem_heapsize() - heapsize_init);
		#endif
	    ANALYSIS_PRINTF("%lu byte freed. memory util = %lu%%\n", trace->block_sizes[index], util);
